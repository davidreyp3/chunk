...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (section === 'discounts') {
      const [rates, totals, products, locs] = await Promise.all([
        rpc('discount_rates', P), rpc('order_counts', P), rpc('discount_products', P),
        select<any>('locations?select=id,name&order=id'),
      ]);
      return json({ rates, totals, products, locations: locs });
    }
    if (section === 'stores') {
